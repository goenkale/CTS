= Camunda - Token Simulation Tutorial
...
The purpose of this tutorial is to understand what can be simulated in a process definition and why should we include it in our tests.
...
=== Why using tokens to simulate?

Running process instances requiere, for example, some input data, human intervention, conexion to external information systems... *Tokens will go free through the process flow with no requirements*. This is useful to validate our process instance expectations.


WARNING: *DISCLAIMER* _Token simulation is not based on time nor in frequence. It won't return any time or frequence metrics._
...
image::processes/start-simulation.gif[title="Start a Token Simulation"]

==== Check the Token Simulation log

image::processes/play.gif[title="Check the Token Simulation log"]
